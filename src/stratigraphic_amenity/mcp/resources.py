...
class ResourceRegistry:
    ...
    def from_env(cls, base_dir: str | Path | None = None) -> "ResourceRegistry":
        ...
        data_root = _resolve_path(os.getenv("GEOMAP_DATA_ROOT", str(default_data_root())), root)
        cache_root = _resolve_path(
            os.getenv("GEOMAP_CACHE_ROOT", str(default_cache_root())), root
        )
    ...
    def _load(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise McpToolError(
                "registry_corrupt",
                "Persistent MCP registry is invalid JSON; refusing to discard registered state.",
                details={"registry": "mcp/v1/registry.json"},
                recovery_hints=[
                    "Back up and remove the registry file under the configured cache root, then re-register maps."
                ],
            ) from exc
